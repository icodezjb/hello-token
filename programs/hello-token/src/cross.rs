...
impl NormalizedSwapData {
    ...
    pub fn encode_normalized_swap_data(swap_data: &mut Vec<NormalizedSwapData>) -> Vec<u8> {
        let mut data = Vec::<u8>::new();
        swap_data.reverse();
        ...
        while let Some(d) = swap_data.pop() {
...
pub mod test {
    ...
    fn test_serde_swap_data() {
        ...
        let encode_data = NormalizedSwapData::encode_normalized_swap_data(&mut swap_data.clone());
